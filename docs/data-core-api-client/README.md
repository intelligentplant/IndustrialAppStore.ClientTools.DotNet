...
> Note that this means that it may not be possible to pass through the identity of the calling user to calls to the on-premises Data Core API. It may be required to apply your own authorization scheme when running in on-premises mode in order to restrict access to the data sources configured in the Data Core API.
